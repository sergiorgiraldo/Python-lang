...
class Reviews:
    ...
    def deactivate_others(self, cycle):
        self.store.find(ReviewCycle, ReviewCycle.review_cycle != cycle).set(is_active = False)
        self.store.commit()
    ...
    def review_user(self, user, auth_key, status):
        self.store.find(UserReview, UserReview.user_corp_key == user, UserReview.auth_key == auth_key).set(status = status, date_received = datetime.now())

        if status in ['n']:
            self.store.find(UserList, UserList.user_corp_key == user).set(is_active = False)

        self.store.commit()

    def handle_active_review(self):
        current_cycle = self.store.find(ReviewCycle, ReviewCycle.is_active == True).any().review_cycle
        reviews = self.store.find(UserReview, UserReview.review_cycle == current_cycle)
        for review in reviews:
            if review.date_received is None:
                if datetime.now().date() - review.date_sent > timedelta(days=15):
                    self.store.find(UserList, UserList.user_corp_key == review.user_corp_key).set(is_active = False)
        self.store.commit()
...
@app.route('/create_review_cycle', methods=['PUT'])
...
@app.route('/handle_reviews', methods=['POST'])
def handle_reviews():
    try:
        r = Reviews()

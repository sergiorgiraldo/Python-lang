from parking import *
from datetime import datetime

def test_AddReservation():
    P = Parking()
    firstReservation = ParkingReservation("F1234", "A1", datetime(2022, 7, 4, 11, 30), datetime(2022, 7, 4, 12, 30))
    P.AddReservation(firstReservation)
    assert len(P.reservations) == 1

def test_AvailabilityFull():
    P = Parking()
    P.reservations.clear()
    assert P.CountAvailableSpots() == len(P.Spots)

def test_AvailabilityJustOne():
    P = Parking()
    firstReservation = ParkingReservation("F1234", "A1", datetime(2022, 7, 4, 11, 30), datetime(2022, 7, 4, 12, 30))
    P.AddReservation(firstReservation)
    assert P.CountAvailableSpots() == (len(P.Spots) - 1)

def test_AvailabilityHasReservationForTomorrow():
    P = Parking()
    firstReservation = ParkingReservation("F1234", "A1", datetime(2022, 7, 5, 11, 30), datetime(2022, 7, 5, 12, 30))
    P.AddReservation(firstReservation)
    assert len(P.reservations) == 1
    assert P.CountAvailableSpots() == len(P.Spots)

def test_AvailabilityCheckSpot():
    P = Parking()
    assert "A1" in P.ListAvailableSpots()
    firstReservation = ParkingReservation("F1234", "A1", datetime(2022, 7, 4, 11, 30), datetime(2022, 7, 4, 12, 30))
    P.AddReservation(firstReservation)
    assert "A1" not in P.ListAvailableSpots()

def test_ValidationAcceptSamePlateSameSpot():
    P = Parking()
    firstReservation = ParkingReservation("F1234", "A1", datetime(2022, 7, 4, 11, 30), datetime(2022, 7, 4, 12, 30))
    msg = P.AddReservation(firstReservation)
    assert (msg == "")

    secondReservation = ParkingReservation("F1234", "A1", datetime(2022, 7, 4, 13, 30), datetime(2022, 7, 4, 14, 30))
    msg = P.AddReservation(secondReservation)
    assert (msg == "")

    assert len(P.reservations) == 2

def test_ValidationAcceptDifferentPlateDifferentSpot():
    P = Parking()
    firstReservation = ParkingReservation("F1234", "A1", datetime(2022, 7, 4, 11, 30), datetime(2022, 7, 4, 12, 30))
    msg = P.AddReservation(firstReservation)
    assert (msg == "")

    secondReservation = ParkingReservation("F5678", "A2", datetime(2022, 7, 4, 11, 30), datetime(2022, 7, 4, 12, 30))
    msg = P.AddReservation(secondReservation)
    assert (msg == "")

    assert len(P.reservations) == 2

def test_ValidationReject():
    P = Parking()
    firstReservation = ParkingReservation("F1234", "A1", datetime(2022, 7, 4, 11, 30), datetime(2022, 7, 4, 12, 30))
    msg = P.AddReservation(firstReservation)
    assert (msg == "")

    secondReservation = ParkingReservation("F5678", "A1", datetime(2022, 7, 4, 13, 30), datetime(2022, 7, 4, 14, 30))
    msg = P.AddReservation(secondReservation)
    assert (not msg == "")
    
    assert len(P.reservations) == 1


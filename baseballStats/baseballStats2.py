...
train_pythagorean = np.empty([0, 1])
...
for year in range(2011, 2016):
    ...
    train_pythagorean = np.vstack([train_pythagorean, df_proc['Pythagorean expectation'].values.reshape([-1, 1])])    

 # Instantiate an object
lin_regr = linear_model.LinearRegression(fit_intercept=False)

# Compute model parameters with training data
lin_regr.fit(train_run_diff, train_pythagorean)
...
r_squared = lin_regr.score(train_run_diff, train_pythagorean)
...
plt.plot(train_run_diff, train_pythagorean, 'bo', label="training_data")
plt.plot(x_ax, y_ax, 'r', label="model_fit")
plt.plot([-300, 300], [1, 1], "k--")
